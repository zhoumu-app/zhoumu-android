...
- **18 项单元测试全过**（断言和 iOS 版 `Tools/CalculatorCheck` 一一对应）
- **Debug APK 编译通过**（minSdk 26 / targetSdk 35）
- **在 Android 15 模拟器上跑起来过**，首页和设置页都正常（见上面两张图）

还没做的：

- 桌面小组件和上下课提醒**没有实际验证过** ——
  这两块要真机或者装 App 到桌面才能试，模拟器上只验证了编译通过。
